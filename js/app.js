    ...
    (() => {
        ...
        class SelectConstructor {
            ...
            getSelectTitleValue(selectItem, originalSelect) {
                ...
                const selectPlaceholder = this.getSelectPlaceholder(originalSelect);
                const labelHTML = selectPlaceholder && selectPlaceholder.label.show ? `<span class="${this.selectClasses.classSelectLabel}">${selectPlaceholder.label.text ? selectPlaceholder.label.text : selectPlaceholder.value}</span>` : "";
                ...
                if (originalSelect.hasAttribute("data-search")) return `<div class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-placeholder="${selectTitleValue}" class="${this.selectClasses.classSelectInput}"></span></div>`; else {
                    ...
                    return `<button type="button" class="${this.selectClasses.classSelectTitle}"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}">${labelHTML}<span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span></button>`;
                }
            }
        ...
        document.addEventListener("click", (function(e) {
            ...
            const its_menuBody = target == menu || menu.contains(target);
            const its_menuOpenButton = target == openMenu || openMenu.contains(target);
            const menuBody_is_active = menu.classList.contains("active");
            if (!its_menuBody && !its_menuOpenButton && menuBody_is_active) {
                menu.classList.remove("active");
                document.documentElement.classList.remove("lock");
            }
        }));
        ...
        document.addEventListener("DOMContentLoaded", (() => {
            const infoBodies = document.querySelectorAll(".info__body");
            if (!infoBodies.length) return;
            for (const infoBody of infoBodies) {
        ...
        const modalOpenButtons = document.querySelectorAll(".modal-cart-open");
        if (modalOpenButtons.length) {
            ...
            const phoneInput = modalForm.querySelector('[name="phone"]');
            const timeSelect = modalForm.querySelector('[name="time"]');
            const timeSelectWrapper = timeSelect.parentElement;
            for (const openButton of modalOpenButtons) openButton.addEventListener("click", (() => {
                modal.classList.add("popup_show");
                modal.setAttribute("aria-hidden", "false");
                document.documentElement.classList.add("lock");
                document.documentElement.classList.add("popup_show");
            }));
            function closeModal() {
                modal.classList.remove("popup_show");
                modal.setAttribute("aria-hidden", "true");
                document.documentElement.classList.remove("lock");
                document.documentElement.classList.remove("popup_show");
            }
            ...
            document.addEventListener("keydown", (e => {
                if (e.key === "Escape" && modal.classList.contains("popup_show")) closeModal();
            }));
            function validateForm() {
                ...
                if (!timeSelect.value) {
                    timeSelectWrapper.classList.add("error");
                    isValid = false;
                } else timeSelectWrapper.classList.remove("error");
                return isValid;
            }
            function clearForm() {
                modalForm.reset();
                modules_flsModules.select.selectBuild(timeSelect);
                nameInput.classList.remove("error");
                phoneInput.classList.remove("error");
                timeSelectWrapper.classList.remove("error");
            }
            modalForm.addEventListener("submit", (e => {
                e.preventDefault();
                if (!validateForm()) return;
                const formValues = Object.fromEntries(new FormData(modalForm));
                closeModal();
                console.log(formValues);
                clearForm();
            }));
            const allInputs = modalForm.querySelectorAll(".input");
            ...
            document.addEventListener("selectCallback", (e => {
                if (e.detail.select === timeSelect) timeSelectWrapper.classList.remove("error");
            }));
        }
